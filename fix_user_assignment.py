...
async def fix_user():
    ...
    try:
        ...
        if not user:
            print(f"FAILED: User {user_email} not found!")
            return
        ...
        tenants = await conn.fetch("SELECT id, name, primary_color, secondary_color FROM tenants ORDER BY id")
        print("\n[AVAILABLE COMPANIES] in Database:")
        for t in tenants:
            print(f"  ID: {t['id']} | Name: {t['name']} | Colors: {t['primary_color']}, {t['secondary_color']}")
            
        # 2. Fix if needed
        if user['tenant_id'] == 1:
            print(f"\nUser is currently assigned to Inferth Mapping (#1).")
            ...
            print(f"\nSUCCESS: {user_email} is already assigned to Company ID {user['tenant_id']}.")
