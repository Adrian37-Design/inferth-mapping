...
@app.get("/create-super-admin-777")
async def create_super_admin_magic():
    """
    Temporary endpoint to create the admin user if CLI is inaccessible.
    """
    from app.db import AsyncSessionLocal
    from app.models import User, Tenant
    from app.security import hash_password
    ...
    async with AsyncSessionLocal() as db:
        ...
        tenant = res.scalars().first()
        if not tenant:
            tenant = Tenant(name="Inferth Mapping", primary_color="#2D5F6D", secondary_color="#EF4835")
            db.add(tenant)
            await db.commit()
            await db.refresh(tenant)
            results["tenant"] = f"Created Tenant: {tenant.name} (ID: {tenant.id})"
        else:
            results["tenant"] = f"Tenant exists: {tenant.name} (ID: {tenant.id})"
            
        # 2. Create Adrian (The User in the Screenshot)
        res = await db.execute(select(User).where(User.email == "adriankwaramba@gmail.com"))
        existing_adrian = res.scalars().first()
        if not existing_adrian:
            adrian = User(
            ...
            db.add(adrian)
            await db.commit()
            results["adrian"] = "Created User: adriankwaramba@gmail.com / Kingcarter@1"
        else:
            results["adrian"] = "User already exists: adriankwaramba@gmail.com"

        # 3. Create Fallback Admin
        ...
        if not existing_admin:
            ...
            db.add(admin_user)
            await db.commit()
            results["fallback_admin"] = "Created User: admin@inferth.com / admin123"
        else:
            results["fallback_admin"] = "User already exists: admin@inferth.com"
